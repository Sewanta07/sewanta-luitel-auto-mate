    ...
    <main class="sf-cus-main">
        <div class="sf-cus-head">
            <div>
                ...
                <p class="sf-cus-subtitle">Customers with bookings assigned to you ({{ $customers->count() }} total)</p>
            </div>
        ...
        <div class="sf-cus-grid" id="customer-list">
            @foreach($customers as $customer)
                ...
                @endphp
                <div class="sf-cus-card customer-card" data-customer-name="{{ strtolower($customer->name) }}">
                    <div class="sf-cus-card-body">
                        <div class="sf-cus-card-head">
                            <div class="sf-cus-person">
                                <div class="sf-cus-avatar sf-cus-avatar-{{ $color }}">
                                    {{ $initials }}
                                </div>
                                <div class="sf-cus-person-meta">
                                    <h3 class="sf-cus-person-name">{{ $customer->name }}</h3>
                                    @if($latestBooking)
                                        <p class="sf-cus-person-vehicle">{{ $latestBooking->vehicle_model }} ({{ $latestBooking->vehicle_number }})</p>
                                    @endif
                                </div>
                            </div>
                            @if($latestBooking)
                                <span class="sf-cus-status
                                    @if($latestBooking->status == 'In Progress') sf-cus-status-progress
                                    @elseif($latestBooking->status == 'Completed') sf-cus-status-completed
                                    @elseif($latestBooking->status == 'Pending') sf-cus-status-pending
                                    @else sf-cus-status-default
                                    @endif">
                        ...
                        <div class="sf-cus-details">
                            <div class="sf-cus-detail-grid">
                                <div>
                                    ...
                                    <span class="sf-cus-detail-value">{{ $customer->phone ?? 'N/A' }}</span>
                                </div>
                                <div>
                                    <span class="sf-cus-detail-label">Email</span>
                                    <span class="sf-cus-detail-value sf-cus-detail-truncate" title="{{ $customer->email }}">{{ $customer->email }}</span>
                                </div>
                            </div>
                            <div class="sf-cus-bookings-meta">
                                <span class="sf-cus-detail-label sf-cus-bookings-label">Bookings</span>
                                <span class="sf-cus-detail-value">{{ $customer->bookings->count() }} service(s)</span>
                            </div>
                        </div>
                        <div class="sf-cus-card-action-wrap">
                            <a href="{{ route('staff.customers.messages', $customer->id) }}" class="sf-cus-message-btn">
                                ...
                                @if($customer->unread_count > 0)
                                    <span class="sf-cus-unread-badge">{{ $customer->unread_count }}</span>
                                @endif
                            </a>
                        </div>
                    </div>
