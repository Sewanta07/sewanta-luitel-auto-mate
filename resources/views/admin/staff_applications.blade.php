                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                @foreach($pendingStaff as $staff)
                                    <tr class="hover:bg-gray-50 transition">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $staff->created_at?->format('M d, Y') }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <form action="{{ route('admin.staff-applications.updateRole', $staff) }}" method="POST" class="flex items-center gap-2">
                                                @csrf
                                                <input type="text" name="level" class="block w-32 rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-xs py-1 px-2" placeholder="e.g. Senior Mech" value="{{ $staff->position ?? '' }}">
                                                <button type="submit" class="text-indigo-600 hover:text-indigo-900 text-xs font-bold">Save</button>
                                            </form>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                            <div class="flex items-center justify-end gap-3">
                                                <form action="{{ route('admin.staff-applications.approve', $staff) }}" method="POST">
                                                    @csrf
                                                    <button type="submit" class="text-green-600 hover:text-green-900 font-bold">Approve</button>
                                                </form>
                                                <form action="{{ route('admin.staff-applications.reject', $staff) }}" method="POST">
                                                    @csrf
                                                    <button type="submit" class="text-red-500 hover:text-red-700 font-bold">Reject</button>
                                                </form>
                                                <form action="{{ route('admin.staff-applications.destroy', $staff) }}" method="POST" onsubmit="return confirm('Delete this application permanently?');">
                                                    @csrf
                                                    @method('DELETE')
