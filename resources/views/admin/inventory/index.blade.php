        ...
        <main class="max-w-7xl w-full mx-auto p-6 space-y-8">
            ...
            <div class="flex items-center justify-between">
                ...
                <div class="flex gap-3">
                    <a href="{{ route('admin.inventory.reports') }}" class="px-4 py-3 rounded-lg border-2 border-gray-300 text-gray-700 hover:border-orange-500 hover:text-orange-600 transition font-bold flex items-center gap-2">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z"/></svg>
                        Reports
                    </a>
                    <a href="{{ route('admin.inventory.create') }}" class="px-4 py-3 rounded-lg bg-gradient-to-r from-orange-500 to-orange-600 text-white hover:shadow-lg transition font-bold flex items-center gap-2 shadow-lg">
                    ...
                    <table class="w-full text-left">
                        ...
                        <tbody class="divide-y divide-gray-100">
                            @forelse($items as $item)
                                @php
                                    $stockPercent = min(($item->quantity / max($item->minimum_stock, 1)) * 100, 100);
                                ...
                                <tr class="hover:bg-gray-50 transition-colors group">
                                    <td class="px-6 py-4">
                                        <p class="font-bold text-gray-900 group-hover:text-orange-600">{{ $item->part_name }}</p>
                                    </td>
                                    <td class="px-6 py-4">
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="flex items-center gap-3">
                                            <div class="w-20 bg-gray-200 rounded-full h-2.5">
                                                <div class="bg-gradient-to-r {{ $item->stock_status === 'out_of_stock' ? 'from-red-500 to-red-600' : ($item->stock_status === 'low_stock' ? 'from-orange-500 to-orange-600' : 'from-green-500 to-green-600') }} h-2.5 rounded-full {{ $stockWidthClass }}"></div>
                                            </div>
                                            <span class="text-sm font-bold text-gray-900 whitespace-nowrap">{{ $item->quantity }}/{{ $item->minimum_stock }}</span>
                                        </div>
                                    </td>
                                    <td class="px-6 py-4">
                                    ...
                                    <td class="px-6 py-4 text-right">
                                        <div class="flex items-center justify-end gap-2">
                                            <a href="{{ route('admin.inventory.edit', $item->id) }}" class="inline-flex items-center gap-1 px-3 py-2 rounded-lg bg-orange-50 text-orange-600 hover:bg-orange-100 transition font-bold text-sm">
                                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                                                Edit
                                            </a>
                                            <form action="{{ route('admin.inventory.destroy', $item->id) }}" method="POST" class="inline" onsubmit="return confirm('Are you sure you want to delete this item? This action cannot be undone.');">
                                                @csrf
                                                @method('DELETE')
