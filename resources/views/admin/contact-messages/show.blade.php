        ...
        <main class="flex-1 max-w-4xl w-full mx-auto p-4 sm:p-6 lg:p-8">
            ...
            <div class="mb-8 mt-4">
                <div class="flex items-center text-sm text-gray-500 mb-2">
                    <a href="{{ route('admin.contact-messages.index') }}" class="hover:text-[#ff5a1f] transition-colors">Contact Messages</a>
                ...
                <div class="px-6 py-4 bg-gray-50 border-b border-gray-200">
                    <div class="flex items-center justify-between">
                        ...
                        <div class="text-right">
                            ...
                            @endif
                            <p class="text-xs text-gray-500 mt-1">{{ $message->created_at->format('M d, Y h:i A') }}</p>
                        </div>
                    </div>
                </div>
            ...
            <div class="bg-white rounded-3xl shadow-sm border border-gray-100 p-6">
                ...
                <div class="flex flex-wrap gap-3">
                    {{-- Update Status --}}
                    <form action="{{ route('admin.contact-messages.updateStatus', $message->id) }}" method="POST" class="inline-flex">
                        @csrf
                    ...
                    {{-- Email Reply --}}
                    <a href="mailto:{{ $message->email }}?subject=Re: {{ $message->subject }}" class="inline-flex items-center px-4 py-2 rounded-xl bg-[#ff5a1f] text-white font-semibold hover:bg-[#e64b15] transition">
                    ...
                    {{-- Delete --}}
                    <form action="{{ route('admin.contact-messages.destroy', $message->id) }}" method="POST" class="inline-flex" onsubmit="return confirm('Are you sure you want to delete this message?');">
                        @csrf
                        @method('DELETE')
