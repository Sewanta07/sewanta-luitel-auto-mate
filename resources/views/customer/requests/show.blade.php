  ...
  <main class="max-w-7xl mx-auto p-6">
    ...
    <div class="flex items-center justify-between mb-8">
      <div>
        ...
        <p class="text-gray-400 font-bold mt-2 uppercase tracking-widest text-[10px]">Reference: #BK-{{ str_pad($booking->id, 4, '0', STR_PAD_LEFT) }}</p>
      </div>
      <div class="px-6 py-3 rounded-2xl text-xs font-black uppercase tracking-widest
        {{ $booking->status == 'Completed' ? 'bg-green-50 text-green-600' : 'bg-orange-50 text-orange-500' }}">
        {{ $booking->status }}
      </div>
    </div>
          ...
          <div class="space-y-10">
            @forelse($booking->logs()->with('user')->latest()->get() as $log)
                <div class="flex gap-8 relative group">
                    ...
                    @endif
                    <div class="flex-shrink-0 w-12 h-12 rounded-2xl {{ $log->status == 'Completed' ? 'bg-green-500 text-white shadow-green-100' : 'bg-orange-500 text-white shadow-orange-100' }} flex items-center justify-center font-black text-sm z-10 shadow-lg">
                        {{ substr($log->status, 0, 1) }}
                    </div>
                    <div class="flex-1 pb-2">
                        <div class="flex items-center justify-between mb-3">
                            <h3 class="text-lg font-black text-gray-900 tracking-tight transition-colors group-hover:text-[#ff5a1f]">{{ $log->status }}</h3>
                            <span class="text-[10px] font-black text-gray-300 uppercase tracking-[0.2em]">{{ $log->created_at->format('M d, H:i') }}</span>
                        </div>
            ...
            <div class="bg-white rounded-[2.5rem] shadow-sm border border-gray-100 p-10">
                ...
                <div class="space-y-6">
                    <div>
                        ...
                        <p class="text-lg font-black text-gray-900 tracking-tight">{{ $booking->service_type }}</p>
                    </div>
                    <div>
                        <p class="text-[10px] font-black text-gray-300 uppercase tracking-widest mb-1">Description</p>
                        <p class="text-sm font-bold text-gray-500 italic">"{{ $booking->problem_description ?? 'Standard service request' }}"</p>
                    </div>
                </div>
            </div>
                ...
                <div class="space-y-6">
                    <div class="flex items-center">
                        <div class="w-12 h-12 rounded-2xl bg-orange-50 flex items-center justify-center text-[#ff5a1f] font-black text-lg mr-4">
                            {{ substr($booking->vehicle_model, 0, 1) }}
                        </div>
                        <div>
                            <p class="text-lg font-black text-gray-900 tracking-tight leading-none">{{ $booking->vehicle_model }}</p>
                            <p class="text-[10px] font-black text-gray-300 uppercase tracking-widest mt-2">{{ $booking->vehicle_number }}</p>
                        </div>
                    </div>
                    <div class="pt-4 border-t border-gray-50">
                        <p class="text-[10px] font-black text-gray-300 uppercase tracking-widest mb-1">Category</p>
                        <p class="text-sm font-black text-gray-900">{{ $booking->vehicle_type ?? 'N/A' }}</p>
                    </div>
                </div>
            </div>
        </div>
      ...
      <div class="space-y-10">
        ...
        <div class="bg-white rounded-[3rem] shadow-xl shadow-gray-100 border border-gray-50 p-10">
          ...
          <div class="flex items-center space-x-5 mb-8">
            ...
            <div>
              <p class="text-xl font-black text-gray-900 tracking-tight leading-none">{{ $booking->staff->name }}</p>
          ...
          <div class="space-y-4">
            <a href="mailto:{{ $booking->staff->email }}" class="flex items-center bg-gray-50 p-4 rounded-2xl text-gray-600 hover:bg-orange-50 hover:text-[#ff5a1f] transition-all group">
              ...
              <span class="text-xs font-black tracking-tight truncate">{{ $booking->staff->email }}</span>
            </a>
          </div>
        </div>
        @else
