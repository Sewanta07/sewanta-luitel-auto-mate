...
<div class="min-h-screen bg-gray-50 pb-12">
    <main class="max-w-7xl mx-auto p-4 sm:p-6 lg:p-8">
        ...
        <form method="GET" action="{{ route('customer.history') }}" class="bg-white rounded-2xl shadow-sm border border-gray-100 p-4 mb-6">
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                <div>
                    ...
                    <select id="vehicle" name="vehicle" class="block w-full px-4 py-2.5 rounded-xl border-gray-200 focus:border-[#ff5a1f] focus:ring-[#ff5a1f] text-sm transition cursor-pointer">
                        ...
                        @foreach($vehicleOptions as $vehicle)
                            <option value="{{ $vehicle->vehicle_number }}" {{ request('vehicle') === $vehicle->vehicle_number ? 'selected' : '' }}>
                                {{ $vehicle->vehicle_model }}{{ $vehicle->vehicle_year ? ' (' . $vehicle->vehicle_year . ')' : '' }}
                            </option>
                        @endforeach
                    </select>
                </div>
                <div>
                    ...
                    <select id="service" name="service" class="block w-full px-4 py-2.5 rounded-xl border-gray-200 focus:border-[#ff5a1f] focus:ring-[#ff5a1f] text-sm transition cursor-pointer">
                        ...
                        @foreach($serviceOptions as $serviceType)
                            <option value="{{ $serviceType }}" {{ request('service') === $serviceType ? 'selected' : '' }}>
                                {{ $serviceType }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="md:col-span-2 relative">
                    ...
                    <input type="text" name="q" value="{{ request('q') }}" placeholder="Search by booking, vehicle, service..." class="block w-full pl-10 pr-4 py-2.5 rounded-xl border-gray-200 focus:border-[#ff5a1f] focus:ring-[#ff5a1f] text-sm transition">
                </div>
            </div>
            <div class="mt-4 flex justify-end gap-2">
                <a href="{{ route('customer.history') }}" class="px-4 py-2 rounded-lg border border-gray-200 text-sm font-semibold text-gray-700 hover:bg-gray-50 transition">Reset</a>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
            @if($history->isEmpty())
                <div class="flex flex-col items-center justify-center py-20 px-4 text-center">
                    ...
                    <a href="{{ route('bookings.create') }}" class="px-6 py-2 rounded-xl bg-[#ff5a1f] text-white font-bold hover:bg-[#e64b15] transition">
                        Request a Service
                    </a>
                </div>
            @else
                        ...
                        <tbody class="divide-y divide-gray-50">
                            @foreach($history as $item)
                                <tr class="hover:bg-gray-50/50 transition">
                                    <td class="px-6 py-4">
                                        <p class="text-sm font-bold text-gray-900">{{ $item->booking_code ?? ('#' . $item->id) }}</p>
                                        <p class="text-xs text-gray-500">{{ $item->preferred_time_slot ?? 'N/A' }}</p>
                                    </td>
                                    <td class="px-6 py-4">
                                        <p class="text-sm font-medium text-gray-900">{{ $item->vehicle_model }}</p>
                                        <p class="text-xs text-gray-500 font-mono">{{ $item->vehicle_number }}</p>
                                    </td>
                                    <td class="px-6 py-4">
                                    ...
                                    <td class="px-6 py-4 text-sm text-gray-700">
                                        {{ $item->staff->name ?? 'Not Assigned' }}
                                    </td>
                                    <td class="px-6 py-4 text-right">
                                        <div class="inline-flex items-center gap-2">
                                            <a href="{{ route('bookings.show', $item->id) }}" class="inline-flex items-center px-3 py-1.5 rounded-lg border border-[#ff5a1f] text-[#ff5a1f] text-xs font-bold hover:bg-orange-50 transition">
                                                View Details
                                            </a>
                                            <a href="{{ route('bookings.invoice', $item->id) }}" class="inline-flex items-center px-3 py-1.5 rounded-lg border border-gray-200 text-gray-700 text-xs font-bold hover:bg-gray-50 transition">
                                                Invoice
                                            </a>
                                        </div>
                                    </td>
