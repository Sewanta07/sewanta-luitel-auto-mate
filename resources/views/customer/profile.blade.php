            ...
            <div class="cs-alert cs-alert-error mb-6 p-4 rounded-2xl bg-red-50 border border-red-100 text-red-800">
                ...
                <ul class="cs-alert-list list-disc pl-5 space-y-1 text-sm">
                    @foreach($errors->all() as $error)
                        <li class="cs-alert-list-item">{{ $error }}</li>
                    @endforeach
                </ul>
            </div>
        @endif
        ...
        <div class="cs-profile-grid grid grid-cols-1 gap-8">
            ...
            <div class="cs-profile-sidebar space-y-8">
                ...
                <div class="cs-profile-card bg-white rounded-3xl shadow-sm border border-gray-100 p-6 text-center">
                    ...
                    <form action="{{ route('customer.profile.update') }}" method="POST" enctype="multipart/form-data" class="cs-profile-photo-form">
                        @csrf
                        <div class="cs-profile-avatar-wrap relative inline-block">
                            <div class="cs-profile-avatar w-32 h-32 rounded-full overflow-hidden border-4 border-orange-50 mx-auto bg-gray-100 flex items-center justify-center">
                                ...
                                @else
                                    <span class="cs-profile-avatar-fallback text-4xl font-bold text-gray-300">{{ strtoupper(substr($customer->name ?? 'U', 0, 1)) }}</span>
                                @endif
                            </div>
                ...
                <div class="cs-profile-card bg-white rounded-3xl shadow-sm border border-gray-100 p-6">
                    ...
                    <form action="{{ route('customer.profile.password') }}" method="POST" class="cs-security-form space-y-4">
                        @csrf
            ...
            <div class="cs-profile-content space-y-8">
                ...
                <div class="cs-profile-card bg-white rounded-3xl shadow-sm border border-gray-100 p-6 sm:p-8">
                    ...
                    <form action="{{ route('customer.profile.update') }}" method="POST" class="cs-personal-form grid grid-cols-1 md:grid-cols-2 gap-6">
                        @csrf
                        <div class="cs-field-group cs-field-group-full md:col-span-2">
                            ...
                            <input type="text" name="name" value="{{ old('name', $customer->name) }}" required class="cs-field-input block w-full px-4 py-3 rounded-xl border-gray-200 focus:border-[#ff5a1f] focus:ring-[#ff5a1f] transition duration-200">
                        </div>
                        <div class="cs-field-group">
                            ...
                            <input type="email" name="email" value="{{ old('email', $customer->email) }}" required class="cs-field-input block w-full px-4 py-3 rounded-xl border-gray-200 focus:border-[#ff5a1f] focus:ring-[#ff5a1f] transition duration-200">
                        </div>
                        <div class="cs-field-group">
                            <label class="cs-field-label block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2">Phone Number</label>
                            <input type="tel" name="phone" value="{{ old('phone', $customer->phone) }}" class="cs-field-input block w-full px-4 py-3 rounded-xl border-gray-200 focus:border-[#ff5a1f] focus:ring-[#ff5a1f] transition duration-200">
                        </div>
                        <div class="cs-field-group cs-field-group-full md:col-span-2">
                            <label class="cs-field-label block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2">Mailing Address</label>
                            <input type="text" name="current_address" value="{{ old('current_address', $customer->current_address) }}" class="cs-field-input block w-full px-4 py-3 rounded-xl border-gray-200 focus:border-[#ff5a1f] focus:ring-[#ff5a1f] transition duration-200">
                        </div>
                ...
                <div class="cs-profile-card bg-white rounded-3xl shadow-sm border border-gray-100 p-6 sm:p-8">
                    <div class="cs-vehicles-head flex items-center justify-between mb-8">
                        ...
                        <a href="{{ route('customer.vehicles') }}" class="cs-vehicles-add-link text-sm font-bold text-[#ff5a1f] hover:text-[#e64b15] transition flex items-center">
                                    ...
                                    <tr class="cs-vehicles-row hover:bg-gray-50/50 transition">
                                        <td class="cs-vehicles-vehicle-cell px-4 py-4">
                                            <div class="cs-vehicle-summary flex items-center">
                                                ...
                                                <span class="cs-vehicle-name text-sm font-bold text-gray-900">{{ $vehicle->vehicle_name ?: ($vehicle->brand . ' ' . $vehicle->model) }}</span>
                                            </div>
                                        </td>
                                        <td class="cs-vehicles-model-cell px-4 py-4 text-sm text-gray-600">{{ $vehicle->model }} / {{ $vehicle->year }}</td>
                                        <td class="cs-vehicles-plate-cell px-4 py-4 text-sm text-gray-600 font-mono">{{ $vehicle->plate_number }}</td>
                                        <td class="cs-vehicles-action-cell px-4 py-4 text-sm">
                                            <div class="cs-vehicle-actions flex items-center space-x-3">
                                                <a href="{{ route('vehicles.edit', $vehicle->id) }}" class="cs-vehicle-action-edit text-blue-600 hover:text-blue-800 transition font-medium">Edit</a>
                                                <form action="{{ route('vehicles.destroy', $vehicle->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to remove this vehicle?');" class="inline">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit" class="cs-vehicle-action-remove text-red-500 hover:text-red-700 transition font-medium" {{ $vehicle->rented_by_user_id ? 'disabled' : '' }}>
                                                        Remove
                                                    </button>
                                                </form>
                                            </div>
