    ...
    <div class="cs-vehicle-alert cs-vehicle-alert-error col-span-full mb-6 p-4 rounded-2xl bg-red-50 border border-red-100 text-red-800 animate-fade-in">
        ...
        <ul class="cs-vehicle-alert-list list-disc pl-5 space-y-1 text-sm">
            @foreach($errors->all() as $error)
                <li class="cs-vehicle-alert-item">{{ $error }}</li>
            @endforeach
        </ul>
    </div>
@endif
        ...
        <div class="cs-vehicle-card bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-md transition duration-300 flex flex-col">
            <div class="cs-vehicle-card-media relative h-44 bg-gray-100">
                @if($vehicle->image_path)
                    <img src="{{ asset('storage/' . $vehicle->image_path) }}" alt="{{ $vehicle->vehicle_name ?? $vehicle->brand }}" class="cs-vehicle-card-image w-full h-full object-cover">
                @else
                ...
                @endif
                <span class="cs-vehicle-status-chip absolute top-3 right-3 inline-flex items-center px-3 py-1 rounded-full text-xs font-bold {{ $status['badge_bg'] }} {{ $status['badge_text'] }} shadow-sm">
                    <span class="cs-vehicle-status-chip-dot w-1.5 h-1.5 rounded-full {{ $status['dot_color'] }} mr-2"></span>
                    {{ $status['status'] }}
                </span>
            ...
            <div class="cs-vehicle-card-body p-6 flex-1">
                <div class="cs-vehicle-card-summary flex items-start justify-between mb-4">
                    <div class="cs-vehicle-card-icon-wrap {{ $statusToneClass }} p-3 rounded-2xl">
                    ...
                    <div class="cs-vehicle-card-type text-right">
                        <p class="cs-vehicle-card-type-main text-xs text-gray-500">{{ $vehicle->vehicle_type ?? 'Vehicle' }}</p>
                        <p class="cs-vehicle-card-type-sub text-xs text-gray-400">{{ $vehicle->fuel_type ?? 'Fuel N/A' }} • {{ $vehicle->transmission_type ?? 'Transmission N/A' }}</p>
                    </div>
                </div>
                <h3 class="cs-vehicle-card-title text-xl font-bold text-gray-900 mb-1">
                    {{ $vehicle->vehicle_name ?: ($vehicle->brand . ' ' . $vehicle->model) }}
                </h3>
                <p class="cs-vehicle-card-subtitle text-sm text-gray-500 mb-2">{{ $vehicle->brand }} {{ $vehicle->model }} • {{ $vehicle->year }}</p>
                
                @if($vehicle->daily_rate)
                    <p class="cs-vehicle-card-rate text-sm text-[#ff5a1f] font-semibold mb-3">Rs. {{ number_format($vehicle->daily_rate, 2) }} / day</p>
                @endif
            ...
            <div class="cs-vehicle-card-actions px-6 py-4 bg-gray-50/50 border-t border-gray-50 space-y-3">
                <div class="cs-vehicle-card-actions-row flex items-center justify-between">
                    <div class="cs-vehicle-card-actions-left flex items-center space-x-3">
                        <a href="{{ route('vehicles.edit', $vehicle->id) }}" class="cs-vehicle-action-link cs-vehicle-action-edit text-gray-400 hover:text-blue-600 transition" title="Edit Vehicle">
                        ...
                        <form action="{{ route('vehicles.destroy', $vehicle->id) }}" method="POST" class="inline" onsubmit="return confirm('Are you sure you want to remove this vehicle?');">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="cs-vehicle-action-link cs-vehicle-action-remove text-gray-400 hover:text-red-500 transition" title="Remove Vehicle" {{ $vehicle->rented_by_user_id ? 'disabled' : '' }}>
                    ...
                    <a href="{{ route('bookings.create', ['vehicle_id' => $vehicle->id]) }}" class="cs-vehicle-service-link text-sm font-bold text-[#ff5a1f] hover:text-[#e64b15] transition flex items-center">
                        Request Service
                        ...
                        <div class="cs-vehicle-pending-list space-y-2">
                            @foreach($vehicle->rentalRequests as $request)
                                <div class="cs-vehicle-pending-item flex items-center justify-between">
                                    <span class="cs-vehicle-pending-id text-xs text-gray-600">Request #{{ $request->id }}</span>
                ...
                <form action="{{ route('vehicles.toggle-rent', $vehicle->id) }}" method="POST">
                    @csrf
                    <button type="submit" class="cs-vehicle-toggle-rent w-full px-4 py-2 rounded-xl text-sm font-bold transition {{ $vehicle->is_listed_for_rent ? 'bg-purple-100 text-purple-700 hover:bg-purple-200' : 'bg-gray-100 text-gray-700 hover:bg-gray-200' }}" {{ $vehicle->rented_by_user_id ? 'disabled' : '' }}>
...
@else
    <div class="cs-vehicle-empty col-span-full py-20 bg-white rounded-3xl border border-dashed border-gray-200 flex flex-col items-center justify-center text-center px-4">
        ...
        <h3 class="cs-vehicle-empty-title text-2xl font-bold text-gray-900 mb-2">{{ $showAll ? "You haven't added any vehicles yet" : "You haven't listed any vehicles for rent yet" }}</h3>
