    ...
    <main class="max-w-7xl mx-auto p-4 sm:p-6 lg:p-8">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            <div class="bg-gradient-to-br from-blue-500 to-blue-600 text-white rounded-2xl shadow-lg p-6">
                ...
                <p class="text-3xl font-bold">Rs. {{ number_format($summary->total_earned ?? 0, 2) }}</p>
            </div>
            ...
            <div class="bg-gradient-to-br from-orange-500 to-orange-600 text-white rounded-2xl shadow-lg p-6">
                ...
                <p class="text-3xl font-bold">Rs. {{ number_format($summary->commission_deducted ?? 0, 2) }}</p>
            </div>
            ...
            <div class="bg-gradient-to-br from-green-500 to-green-600 text-white rounded-2xl shadow-lg p-6">
                ...
                <p class="text-3xl font-bold">Rs. {{ number_format($summary->withdrawable_balance ?? 0, 2) }}</p>
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @forelse($earnings as $earning)
                                <tr class="hover:bg-gray-50">
                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">#{{ $earning->rental_id }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-green-600 font-bold">Rs. {{ number_format($earning->owner_amount, 2) }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-orange-600">Rs. {{ number_format($earning->commission, 2) }}</td>
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @forelse($withdrawalRequests as $request)
                                <tr class="hover:bg-gray-50">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-bold text-gray-900">Rs. {{ number_format($request->amount, 2) }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-bold
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $request->requested_at->format('M d, Y') }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $request->processed_at ? $request->processed_at->format('M d, Y') : '-' }}</td>
                                </tr>
                            @empty
                                <tr>
                                        ...
                                        <div class="flex items-start justify-between">
                                            <div class="flex-1">
                                                <div class="flex items-center gap-2 mb-2">
                                                    <h4 class="font-bold text-gray-900">{{ $request->vehicle?->vehicle_name ?: ($request->vehicle?->brand . ' ' . $request->vehicle?->model) }}</h4>
                                ...
                                @foreach($recentMarketplaceRentals as $rental)
                                    <div class="bg-white rounded-xl border border-gray-100 p-4 hover:shadow-md transition">
                                        <div class="flex items-start justify-between">
                                            <div class="flex-1">
                                                <div class="flex items-center gap-2 mb-2">
                                                    <h4 class="font-bold text-gray-900">{{ $rental->vehicle?->vehicle_name ?: ($rental->vehicle?->brand . ' ' . $rental->vehicle?->model) }}</h4>
                                                    <span class="px-2 py-1 rounded-full text-xs font-bold
...
<div id="withdrawal-modal-backdrop" class="fixed inset-0 bg-black/50 backdrop-blur-sm z-[60] hidden flex items-center justify-center p-4">
    <div id="withdrawal-modal" class="bg-white rounded-3xl shadow-2xl max-w-md w-full transform scale-95 opacity-0 transition-all duration-300">
        <div class="p-8">
            ...
            <form action="{{ route('owner.withdrawals.request') }}" method="POST" class="space-y-6">
                @csrf
                <div>
                    <label class="block text-sm font-bold text-gray-700 mb-2">Available Balance</label>
                    <div class="bg-green-50 border border-green-200 rounded-xl p-4">
                        <p class="text-2xl font-bold text-green-600">Rs. {{ number_format($summary->withdrawable_balance ?? 0, 2) }}</p>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <p class="mt-1 text-xs text-gray-500">Maximum: Rs. {{ number_format($summary->withdrawable_balance ?? 0, 2) }}</p>
                </div>
