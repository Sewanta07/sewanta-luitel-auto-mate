...
@section('title', 'Owner Earnings Dashboard - AutoMate')
    ...
    <main class="cs-owner-earnings-main max-w-7xl mx-auto p-4 sm:p-6 lg:p-8">
        ...
        <div class="cs-owner-stat-grid grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            <div class="cs-owner-stat-card cs-owner-stat-card-total bg-gradient-to-br from-blue-500 to-blue-600 text-white rounded-2xl shadow-lg p-6">
                ...
                <p class="cs-owner-stat-value text-3xl font-bold">Rs. {{ number_format($summary->total_earned ?? 0, 2) }}</p>
            </div>
            ...
            <div class="cs-owner-stat-card cs-owner-stat-card-commission bg-gradient-to-br from-orange-500 to-orange-600 text-white rounded-2xl shadow-lg p-6">
                ...
                <p class="cs-owner-stat-value text-3xl font-bold">Rs. {{ number_format($summary->commission_deducted ?? 0, 2) }}</p>
            </div>
            ...
            <div class="cs-owner-stat-card cs-owner-stat-card-balance bg-gradient-to-br from-green-500 to-green-600 text-white rounded-2xl shadow-lg p-6">
                ...
                <p class="cs-owner-stat-value text-3xl font-bold">Rs. {{ number_format($summary->withdrawable_balance ?? 0, 2) }}</p>
                @if(($summary->withdrawable_balance ?? 0) > 0)
        ...
        <div class="mb-6">
                        ...
                        <tbody class="cs-owner-table-body bg-white divide-y divide-gray-200">
                            @forelse($earnings as $earning)
                                <tr class="cs-owner-table-row hover:bg-gray-50">
                                    <td class="cs-owner-table-cell px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">#{{ $earning->rental_id }}</td>
                                    <td class="cs-owner-table-cell cs-owner-table-cell-amount px-6 py-4 whitespace-nowrap text-sm text-green-600 font-bold">Rs. {{ number_format($earning->owner_amount, 2) }}</td>
                                    <td class="cs-owner-table-cell cs-owner-table-cell-commission px-6 py-4 whitespace-nowrap text-sm text-orange-600">Rs. {{ number_format($earning->commission, 2) }}</td>
                                    <td class="cs-owner-table-cell px-6 py-4 whitespace-nowrap">
                                        <span class="cs-owner-chip inline-flex items-center px-3 py-1 rounded-full text-xs font-bold
                                            {{ $earning->payout_status === 'paid' ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800' }}">
                    ...
                    <table class="cs-owner-table min-w-full divide-y divide-gray-200">
                        ...
                        <tbody class="cs-owner-table-body bg-white divide-y divide-gray-200">
                            @forelse($withdrawalRequests as $request)
                                <tr class="cs-owner-table-row hover:bg-gray-50">
                                    <td class="cs-owner-table-cell px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">#{{ $request->id }}</td>
                                    <td class="cs-owner-table-cell px-6 py-4 whitespace-nowrap text-sm font-bold text-gray-900">Rs. {{ number_format($request->amount, 2) }}</td>
                                    <td class="cs-owner-table-cell px-6 py-4 whitespace-nowrap">
                                        <span class="cs-owner-chip inline-flex items-center px-3 py-1 rounded-full text-xs font-bold
                                    ...
                                    <td class="cs-owner-table-cell px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $request->requested_at->format('M d, Y') }}</td>
                                    <td class="cs-owner-table-cell px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $request->processed_at ? $request->processed_at->format('M d, Y') : '-' }}</td>
                                </tr>
                            @empty
                                <tr>
            ...
            <div class="cs-owner-tab-panel mt-6" data-panel="rentals">
                <div class="cs-owner-rentals-stack space-y-6">
                    ...
                    @if(isset($recentRentalRequests) && $recentRentalRequests->count() > 0)
                        <div class="cs-owner-rentals-section">
                            ...
                            <div class="cs-owner-rentals-list space-y-3">
                                @foreach($recentRentalRequests as $request)
                                        ...
                                        <div class="flex items-start justify-between">
                                            <div class="flex-1">
                                                <div class="cs-owner-rental-item-head flex items-center gap-2 mb-2">
                                                    <h4 class="cs-owner-rental-item-title font-bold text-gray-900">{{ $request->vehicle?->vehicle_name ?: ($request->vehicle?->brand . ' ' . $request->vehicle?->model) }}</h4>
                                                    <span class="cs-owner-chip px-2 py-1 rounded-full text-xs font-bold
                                                        @if($request->status === 'Completed') bg-gray-200 text-gray-700
                                                ...
                                                <div class="cs-owner-rental-item-grid grid grid-cols-3 gap-3 text-sm">
                                                    <div>
                                                        ...
                                                        <p class="font-bold text-gray-900">{{ $request->renter?->name ?? 'N/A' }}</p>
                                                    </div>
                                                    <div>
                                                        <p class="text-xs text-gray-500">Duration</p>
                                                        <p class="font-bold text-gray-900">{{ $request->number_of_days ?? 0 }} days</p>
                                                    </div>
                                                    <div>
                                                        <p class="text-xs text-gray-500">Amount</p>
                                                        <p class="font-bold text-orange-600">Rs. {{ number_format($request->total_cost ?? 0, 2) }}</p>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                    ...
                    {{-- Marketplace Rentals Section --}}
                    @if(isset($recentMarketplaceRentals) && $recentMarketplaceRentals->count() > 0)
                        <div class="cs-owner-rentals-section">
                            <h3 class="cs-owner-rentals-title text-lg font-bold text-gray-800 mb-3">Recent Marketplace Rentals</h3>
                            <div class="cs-owner-rentals-list space-y-3">
                                @foreach($recentMarketplaceRentals as $rental)
                                    <div class="cs-owner-rental-item bg-white rounded-xl border border-gray-100 p-4 hover:shadow-md transition">
                                        <div class="flex items-start justify-between">
                                            <div class="flex-1">
                                                <div class="cs-owner-rental-item-head flex items-center gap-2 mb-2">
                                                    <h4 class="cs-owner-rental-item-title font-bold text-gray-900">{{ $rental->vehicle?->vehicle_name ?: ($rental->vehicle?->brand . ' ' . $rental->vehicle?->model) }}</h4>
                                                    <span class="cs-owner-chip px-2 py-1 rounded-full text-xs font-bold
                                                        @if($rental->status === 'completed') bg-gray-200 text-gray-700
                                                        @elseif($rental->status === 'confirmed') bg-green-100 text-green-700
                                                ...
                                                <div class="cs-owner-rental-item-grid cs-owner-rental-item-grid-marketplace grid grid-cols-4 gap-3 text-sm">
                                                    <div>
                                                        <p class="text-xs text-gray-500">Renter</p>
                                                        <p class="font-bold text-gray-900">{{ $rental->renter?->name ?? 'N/A' }}</p>
                                                    </div>
                                                    <div>
                                                        <p class="text-xs text-gray-500">Duration</p>
                                                        <p class="font-bold text-gray-900">{{ $rental->number_of_days ?? 0 }} days</p>
                                                    </div>
                                                    <div>
                                                        <p class="text-xs text-gray-500">Owner Earning</p>
                                                        <p class="font-bold text-green-600">Rs. {{ number_format($rental->owner_earning ?? 0, 2) }}</p>
                                                    </div>
                                                    <div>
                                                        <p class="text-xs text-gray-500">Commission</p>
                                                        <p class="font-bold text-orange-600">Rs. {{ number_format($rental->commission_amount ?? 0, 2) }}</p>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                    ...
                    @if((!isset($recentRentalRequests) || $recentRentalRequests->count() === 0) && (!isset($recentMarketplaceRentals) || $recentMarketplaceRentals->count() === 0))
...
<div id="withdrawal-modal-backdrop" class="cs-owner-modal-backdrop fixed inset-0 bg-black/50 backdrop-blur-sm z-[60] hidden flex items-center justify-center p-4">
    <div id="withdrawal-modal" class="cs-owner-modal bg-white rounded-3xl shadow-2xl max-w-md w-full transform scale-95 opacity-0 transition-all duration-300">
        <div class="cs-owner-modal-body p-8">
            ...
            <form action="{{ route('owner.withdrawals.request') }}" method="POST" class="cs-owner-modal-form space-y-6">
                @csrf
                <div class="cs-owner-modal-section">
                    ...
                    <div class="cs-owner-modal-balance bg-green-50 border border-green-200 rounded-xl p-4">
                        <p class="cs-owner-modal-balance-value text-2xl font-bold text-green-600">Rs. {{ number_format($summary->withdrawable_balance ?? 0, 2) }}</p>
                    </div>
                </div>
                ...
                <div class="cs-owner-modal-section">
                    ...
                    <input type="number" name="amount" step="0.01" min="1" max="{{ $summary->withdrawable_balance ?? 0 }}" required 
                    ...
                    <p class="cs-owner-modal-help mt-1 text-xs text-gray-500">Maximum: Rs. {{ number_format($summary->withdrawable_balance ?? 0, 2) }}</p>
                </div>
