...
<div class="min-h-screen bg-gray-50" x-data="{ supportModalOpen: false }">
    <main class="max-w-7xl mx-auto p-6 grid grid-cols-1 lg:grid-cols-4 gap-6">
        ...
        <section class="lg:col-span-3 space-y-6">
            ...
            <div class="p-6 rounded-2xl bg-white shadow-sm flex items-center justify-between">
                ...
                <div class="hidden sm:flex items-center space-x-4">
                    ...
                    <div class="text-sm font-medium text-gray-900">{{ \Carbon\Carbon::parse($user->created_at)->format('M Y') }}</div>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                <a href="{{ route('bookings.create') }}" class="p-4 rounded-xl bg-white shadow-sm hover:shadow-md transition border-l-4" style="border-left-color: #ff5a1f;">
                ...
                <a href="{{ route('bookings.index') }}" class="p-4 rounded-xl bg-white shadow-sm hover:shadow-md transition border-l-4" style="border-left-color: #2563eb;">
                ...
                <a href="{{ route('customer.profile') }}" class="p-4 rounded-xl bg-white shadow-sm hover:shadow-md transition border-l-4" style="border-left-color: #7c3aed;">
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="flex items-center justify-between">
                            @foreach($stages as $stage => $statuses)
                                @php $stageIndex = array_search($stage, array_keys($stages)) + 1; @endphp
                                <div class="flex flex-col items-center flex-1">
                                    ...
                                    {{-- Label --}}
                                    <p class="text-xs font-semibold text-center text-gray-900">{{ $stage }}</p>
                                </div>
                                
                                {{-- Connector Line --}}
                                @if($stageIndex < count($stages))
                        ...
                        <div class="mt-6 p-4 rounded-lg border-l-4" style="border-left-color: #ff5a1f; background-color: #fff7ed;">
                            ...
                            <p class="text-lg font-bold text-gray-900 mt-1">{{ $activeBooking->status }}</p>
                            <p class="text-xs text-gray-500 mt-2">Request {{ $activeBooking->created_at->diffForHumans() }}</p>
                        </div>
                    </div>
                @else
                    ...
                    <div class="p-6 rounded-2xl bg-white shadow-sm">
                        ...
                        <div class="space-y-3">
                            @foreach($recentBookings as $booking)
                                <a href="{{ route('bookings.show', $booking->id) }}" class="flex items-center justify-between p-4 bg-gray-50 hover:bg-gray-100 rounded-xl transition-colors">
                                    <div class="flex items-center gap-4 flex-1">
                                        <div class="w-10 h-10 rounded-full" style="background-color: {{ $booking->status === 'Completed' ? '#d1fae5' : ($booking->status === 'In Progress' ? '#dbeafe' : '#fef3c7') }}; color: {{ $booking->status === 'Completed' ? '#059669' : ($booking->status === 'In Progress' ? '#1e40af' : '#92400e') }};" class="flex items-center justify-center text-sm font-bold">
                                        ...
                                        <div class="flex-1 min-w-0">
                                            <p class="text-sm font-semibold text-gray-900 truncate">{{ $booking->service_type ?? 'Service Request' }}</p>
                                            <p class="text-xs text-gray-500">{{ $booking->created_at->format('M d, Y') }}</p>
                                        </div>
                                    </div>
                                    <span class="px-3 py-1 text-xs font-semibold rounded-full" style="background-color: {{ $booking->status === 'Completed' ? '#d1fae5' : ($booking->status === 'In Progress' ? '#dbeafe' : '#fef3c7') }}; color: {{ $booking->status === 'Completed' ? '#059669' : ($booking->status === 'In Progress' ? '#1e40af' : '#92400e') }};">
                                        {{ $booking->status }}
                                    </span>
                                </a>
                            @endforeach
                        </div>
                        <a href="{{ route('bookings.index') }}" class="mt-4 text-sm text-[#ff5a1f] font-semibold hover:text-[#e44d18] block text-center">View All Bookings →</a>
                    </div>
                @else
                    <div class="p-10 rounded-2xl bg-white shadow-sm text-center">
                        ...
                        <div class="mt-6">
                            <a href="{{ route('bookings.create') }}" class="inline-flex items-center px-4 py-2 text-white rounded-lg shadow transition" style="background-color: #ff5a1f;" onmouseover="this.style.backgroundColor='#e64b15'" onmouseout="this.style.backgroundColor='#ff5a1f'">Request Service</a>
                        </div>
                    </div>
                @endif
            </div>
        ...
        {{-- Right column: sidebar with shortcuts --}}
